...
Word *_next(Cell **sp, Word ***eip) {
  Word *w = **eip, *ow = NULL;
  *eip += 1;
  DBGOUT(DBG_TRACE, "next %p\t%p\t%p\n", *eip, *sp, **sp);
  while(w != NULL && *eip) {
    DBGOUT(DBG_TRACE, "-> %s\t%p\t%p\n", w->name, *sp, **sp);
    *sp -= 1;
    (*sp)->word = w;
    ow = w;
    w = w->code(sp, eip);
...
Word *_docol(Cell **sp, Word ***eip) {
  ...
  //(*sp)->word_list = *eip;
  //*sp += 1;
  // todo need to push nothing to sp, needs ezery vord updated
  Cell *fp = (*sp)->ptr = (*sp)+1;
  Word **neip = w->data.word_list;
  Word *r = _next(sp, &neip);
...
Word debug_level = { "*debug-level*", _dovar, (void *)0, &set_dict };
