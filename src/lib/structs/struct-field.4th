...
def struct-get-field-loop ( name len fields -- field )
  ...
  arg2 arg0 car value-ptr struct-field-name peek arg1 1 + byte-string-equals?/3 IF
    arg0 car return1
  THEN
...
def struct-get-field ( name len struct -- field )
  ...
  dup null? IF drop arg0 type-super peek value-ptr set-arg0 repeat-frame
	    ELSE 3 return1-n
	    THEN
end

def struct-get-field-ptr/4 ( instance name len struct -- ptr )
  arg2 arg1 arg0 value-ptr struct-get-field dup null? IF
    s" No field" error-line/2
    0 4 return1-n
  ELSE
    value-ptr struct-field-offset peek
    arg3 value-ptr int-add 4 return1-n
  THEN
end
...
defproper next-struct-field ( struct : field -- struct-field )
  ...
  ELSE ( look up slot of ToS type / last word's return, emit code to add offset )
    2dup 5 overn value-ptr struct-get-field dup null? UNLESS
...
' NORTH-COMPILE-TIME defined? [IF]
  ...
  : [->] ( struct-entry : field -- code... )
    out-off' value-ptr swap [.]
  ; cross-immediate-as ->

  alias> sys-struct-add-field struct-add-field
[THEN]

( Struct field definitions: )

( todo translate type addresses for output )
...
cell-size 0 * swap
pointer<any> swap
here struct-field cons struct-field value-ptr struct-add-field
...
cell-size 0 * swap
pointer<any> swap
here struct-field cons type value-ptr struct-add-field
...
def struct-create-field ( name name-length type struct )
  ...
  local0 struct-field -> type peek value-ptr dup IF struct . byte-size peek local0 struct-field -> byte-size poke ELSE drop THEN
