...
def test-pointer
  null type-of null assert-equals
  null value-of null assert-equals
end

def test-struct
  value value-of struct-byte-size peek cell-size assert-equals
  null struct-byte-size peek cell-size assert-equals
  struct value-of struct-byte-size peek cell-size 5 * assert-equals
end

def test-struct-field
  struct-field type-of struct assert-equals
  struct-field value-of struct-byte-size peek cell-size 4 * assert-equals
end

def test-struct-manually
s" Printing" write-line/2
...
0x100 vec4-y write-hex-uint nl
0x100 thing-rotation write-hex-uint nl
0x100 thing-rotation time-integrated-velocity write-hex-uint nl
